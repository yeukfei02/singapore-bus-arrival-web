<script lang="ts">
  ...
  const handleBusNumberClick = (busNumber: string) => {
    busServiceNoStore.set(busNumber);
    currentPageStore.set("busMapView");
  };
</script>
      ...
      {#each getBusArrivalResult.busArrival.services as item}
        <div class="container my-4">
          <Card>
            <Content>
              <div
                class="my-2 hoverItem"
                style="font-size: 1.5em; font-weight: bold; text-decoration: underline;"
                on:click={() => handleBusNumberClick(item.busNumber)}
              >
                {item.busNumber}
              </div>
                  ...
                  <Card class="p-3">
                    ...
                    <div class="my-2" style="font-size: 1.2em;">
                      Remaining: <span style="font-weight: bold;"
                        >{isNaN(
                          moment(nextBusItem.estimatedArrival).diff(
                            moment(),
                            "minutes"
                          )
                        )
                    ...
                    {:else if nextBusItem.type.includes("Double Deck")}
                      <div class="d-flex flex-row my-2">
                        <img
                          src="images/bus-double-deck.png"
                          width="40"
                          height="40"
                          alt=""
                        />
                    ...
                    <span
                      ...
                      on:click={() =>
                        handleCheckBusInMapClick(
                          nextBusItem.latitude,
                          nextBusItem.longitude
                        )}>Check bus in map</span
                    >
                  </Card>
                </div>
              {/each}
